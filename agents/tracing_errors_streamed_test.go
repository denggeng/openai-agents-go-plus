...
func TestSingleTurnModelErrorStreamed(t *testing.T) {
	...
	assert.Equal(t, []m{
		{
			...
			"children": []m{
				{
					"type":  "agent",
					"error": m{"message": "Error in agent run", "data": m{"error": "test error"}},
					"data": m{
						"name":        "test_agent",
						"handoffs":    []string{},
						"tools":       []string{},
...
func TestMultiTurnNoHandoffsStreamed(t *testing.T) {
	tracingtesting.Setup(t)
	agents.ClearOpenaiSettings()
...
func TestToolCallErrorStreamed(t *testing.T) {
	tracingtesting.Setup(t)
	agents.ClearOpenaiSettings()
...
func TestMultipleHandoffDoesNotErrorStreamed(t *testing.T) {
	tracingtesting.Setup(t)
	agents.ClearOpenaiSettings()
	...
	result, err := agents.RunStreamed(t.Context(), agent3, "user_message")
	require.NoError(t, err)
	err = result.StreamEvents(func(agents.StreamEvent) error { return nil })
	require.NoError(t, err)
...
func TestMultipleFinalOutputDoesNotErrorStreamed(t *testing.T) {
	tracingtesting.Setup(t)
	agents.ClearOpenaiSettings()
...
func TestHandoffsLeadToCorrectAgentSpansStreamed(t *testing.T) {
	tracingtesting.Setup(t)
	agents.ClearOpenaiSettings()
			...
			"children": []m{
				{
					...
					"children": []m{
						...
						{"type": "generation"},
						{
							...
							"error": m{
								"message": "Multiple handoffs requested",
								"data":    m{"requested_agents": []string{"test_agent_1", "test_agent_2"}},
							},
...
func TestMaxTurnsExceededStreamed(t *testing.T) {
	tracingtesting.Setup(t)
	agents.ClearOpenaiSettings()
	...
	result, err := runner.RunStreamed(t.Context(), agent, "user_message")
	require.NoError(t, err)
	err = result.StreamEvents(func(agents.StreamEvent) error { return nil })
...
func TestInputGuardrailErrorStreamed(t *testing.T) {
	tracingtesting.Setup(t)
	agents.ClearOpenaiSettings()
...
func TestOutputGuardrailErrorStreamed(t *testing.T) {
	tracingtesting.Setup(t)
	agents.ClearOpenaiSettings()
